<template>
	<view class="app">
		...
		<view class="pay-type-list">
			...
			<!-- <view class="type-item" @click="changePayType(3)" v-if="depositMoney>=pay&&pay!=0">
				<text class="icon yticon icon-erjiye-yucunkuan" style="margin-right: 20px;"></text>
				<view class="con">
					<text class="tit">预存款支付</text>
					<text>可用余额 {{depositMoney}}</text>
				</view>
				<label class="radio" >
...
<script>
	...
	export default {
		...
		onLoad(options) {
			//如果OrderId为空，但支付金额不为空，则后台处理产生新的一个支付单号
		...
		methods: {
			getUserDeposit(){
				...
				that.$http({ //调用接口
					...
					that.depositMoney=userInfo.chargeRemainMoney
				})
			},
			...
			confirm: async function() {
				...
				if(this.payType==1){
					...
					if (ua.match(/MicroMessenger/i) == 'micromessenger') { 
						...
						location.href=that.global.website +"/"+ "/wx/order/aliprepay?orderId="+
						that.orderid+"&userId="+userInfo.id+"&payMoney="+that.pay;
					}
