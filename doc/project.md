...
* litemall-wx-api后端服务地址`http://localhost:8090/wx`，数据则来自MySQL
* litemall-admin-api后端服务地址`http://localhost:8090/admin`,数据则来自MySQL
...
8. 采用Maven命令运行本项目的litemall-all
   ...
   打开浏览器，输入
    ```
    http://localhost:8090/wx/index/index
    http://localhost:8090/admin/index/index
    ```
...
以及`apis.js`中的`WxApiRoot`，即小商场服务根地址。
    ...
    // 局域网测试使用
    // var WxApiRoot = 'http://192.168.0.101:8090/wx/';
    // 云平台部署时使用
    // var WxApiRoot = 'http://118.24.0.153:8090/wx/';
    // 云平台上线时使用
...
`org.jinyuanjava.litemall.core`定义litemall-core模块的日志级别
`org.jinyuanjava.litemall.db`定义litemall-core模块的日志级别
`org.jinyuanjava.litemall.wx`定义litemall-wx-api模块的日志级别
`org.jinyuanjava.litemall.admin`定义litemall-admin-api模块的日志级别
`org.jinyuanjava.litemall`而定义litemall所有后端模块的日志级别
...
spring:
  datasource:
    druid:
      url:  jdbc:mysql://localhost:3306/paimai?useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC&allowPublicKeyRetrieval=true&verifyServerCertificate=false&useSSL=false
...
litemall:
  storage:
    ...
    local:
      ...
      address: http://localhost:8090/wx/storage/fetch/
```
...
3. 创建安全组
    ...
    目前允许的端口：8091，8092，8093，8090，80，443，22，3306
    
    注意：
    这里其实只需要8090端口，允许其他端口只是方面开发阶段的测试和调试。
...
3. 启动服务
    ...
    java -jar litemall.jar --server.port=8090
    
4. 测试是否部署成功(xxx.xxx.xxx.xxx是云主机IP）：
    ```
    http://xxx.xxx.xxx.xxx:8090/wx/index/index
    http://xxx.xxx.xxx.xxx:8090/admin/index/index
    http://xxx.xxx.xxx.xxx:8090/#/login
    ```

注意：
> 开发者访问以上三个地址都能成功，但是管理后台点击登录时会报错网络连接不成功。
> 这里很可能是开发者litemall-admin模块的`config/dep.env.js`或者`condig/prod.env.js`
> 没有设置正确的管理后台后端地址，例如这里的`http://xxx.xxx.xxx.xxx:8090/admin`
...
* 用户也可以通过http协议的8090访问后端服务（spring boot）
