<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="有上下架权利" prop="avatar">
        <template slot-scope="scope">
          <el-tag :type="scope.row.ifModifyOnSale ? 'success' : 'error' ">{{ scope.row.ifModifyOnSale ? '是' : '否' }}</el-tag>
    ...
    <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon  label-width="100px" >
        ...
        <el-form-item label="上下架审核" prop="ifModifyOnSale">
          <el-radio-group v-model="dataForm.ifModifyOnSale">
        ...
        <el-form-item label="管理员角色" prop="roleIds">
          <el-select clearable v-model="dataForm.roleIds" multiple placeholder="请选择">
            <el-option
...
<script>
import { listAdmin, createAdmin, updateAdmin, deleteAdmin,readminAdmin } from '@/api/admin'
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      readminAdmin(this.dataForm.id).then(
        (response)=>{
