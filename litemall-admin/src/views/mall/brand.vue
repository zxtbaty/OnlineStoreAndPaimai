<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.sn" clearable class="filter-item" style="width: 200px;" placeholder="请输入品牌商编号"/>
...
<!--      <el-table-column align="center" label="底价" prop="floorPrice"/>-->
      <el-table-column align="center" label="是否启用" prop="enabled">
        <template slot-scope="scope">
          <el-tag :type="scope.row.enabled ? 'success' : 'error' ">{{ scope.row.enabled ? '停用' : '启用' }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon  label-width="100px" >
        <el-form-item label="品牌商编号" prop="sn">
...
<script>
import { listBrand, createBrand, updateBrand, deleteBrand,readBrand } from '@/api/brand'
...
export default {
  ...
  methods: {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          createBrand(this.dataForm)
            ...
            .catch(error => {
              this.$message.error( '失败:'+error.data.errmsg);
            })
        }
      })
    },
    handleUpdate(row) {
      ...
      readBrand(this.dataForm.id).then(
        (response)=>{
