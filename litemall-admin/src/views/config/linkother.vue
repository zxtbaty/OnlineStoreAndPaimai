<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" width="150px" label="链接ID" prop="id" sortable/>

      <el-table-column align="center" min-width="100px" label="链接词" prop="keyword"/>
    ...
    <el-dialog :title="textMap[dialogStatus]" :close-on-click-modal="false" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon  label-width="100px" >
        ...
        <el-form-item label="是否推荐" prop="isHot">
          <el-select clearable v-model="dataForm.isHot" placeholder="请选择">
            <el-option :value="true" label="推荐"/>
            <el-option :value="false" label="普通"/>
          </el-select>
        </el-form-item>
        <el-form-item label="是否默认" prop="isDefault">
          <el-select clearable v-model="dataForm.isDefault" placeholder="请选择">
      ...
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createKeyword(this.dataForm).then(response => {
            this.list.unshift(response.data.list)
