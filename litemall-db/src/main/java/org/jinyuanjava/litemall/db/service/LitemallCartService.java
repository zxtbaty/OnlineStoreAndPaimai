...
import org.jinyuanjava.litemall.db.dao.LitemallCartMapper;
import org.jinyuanjava.litemall.db.domain.LitemallCart;
import org.jinyuanjava.litemall.db.domain.LitemallCartExample;
...
public class LitemallCartService {
    ...
    public void clearCheckGoods(Integer userId,Integer comId) {
        LitemallCartExample example = new LitemallCartExample();
        example.or().andUserIdEqualTo(userId).andCheckedEqualTo(true).andComIdEqualTo(comId);
        LitemallCart cart = new LitemallCart();
        cart.setDeleted(true);
        cartMapper.updateByExampleSelective(cart, example);
    }
    public void clearCheckGoods(Integer userId,Integer comId,Integer id) {
        LitemallCartExample example = new LitemallCartExample();
        example.or().andUserIdEqualTo(userId).andCheckedEqualTo(true).andComIdEqualTo(comId).andIdEqualTo(id);
        LitemallCart cart = new LitemallCart();
        cart.setDeleted(true);
        cartMapper.updateByExampleSelective(cart, example);
    }

    public List<LitemallCart> querySelective(Integer userId, Integer goodsId, Integer page, Integer limit, String sort) {
        LitemallCartExample example = new LitemallCartExample();
...
//    public int deleteByProductIds(List<Integer> productIdList) {
//        LitemallCartExample example = new LitemallCartExample();
//        example.or().andProductIdIn(productIdList);
//        return cartMapper.deleteByExample(example);
//    }

    public int deleteByProductIds(String ruleType, Integer ruleId,List<Integer> productIdList) {
        LitemallCartExample example = new LitemallCartExample();
        LitemallCartExample.Criteria criteria=example.createCriteria();
        criteria.andProductIdIn(productIdList);
        if(ruleType.equals("会员")) {
            criteria.andUserGoodsRuleIdEqualTo(ruleId);
        } else  if(ruleType.equals("品项折扣")) {
            criteria.andRebateRuleIdEqualTo(ruleId);
        } else  if(ruleType.equals("团购")) {
            criteria.andGrouponRuleIdEqualTo(ruleId);
        } else if(ruleType.equals("秒杀")) {
            criteria.andSecRuleIdEqualTo(ruleId);
        }
        return cartMapper.deleteByExample(example);
    }

    public int deleteByProductId(String ruleType, Integer ruleId,Integer productId) {
        LitemallCartExample example = new LitemallCartExample();
        LitemallCartExample.Criteria criteria=example.createCriteria();
