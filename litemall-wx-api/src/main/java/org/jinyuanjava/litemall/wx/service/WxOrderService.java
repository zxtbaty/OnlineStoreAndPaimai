...
import static org.jinyuanjava.litemall.wx.util.WxResponseCode.ORDER_COMMENTED;
import static org.jinyuanjava.litemall.wx.util.WxResponseCode.ORDER_COMMENT_EXPIRED;
import static org.jinyuanjava.litemall.wx.util.WxResponseCode.ORDER_INVALID;
import static org.jinyuanjava.litemall.wx.util.WxResponseCode.ORDER_INVALID_OPERATION;
import static org.jinyuanjava.litemall.wx.util.WxResponseCode.ORDER_UNKNOWN;
...
import org.jinyuanjava.litemall.core.util.DateTimeUtil;
...
//            Integer orderId = JacksonUtil.parseInteger(body, "orderId");
            if (orderId == null) {
                ...
                {
                    return wxToPay(userId,payMoney,request,response);
                }
            }
...
//          orderRequest.setSpbillCreateIp(IpUtil.getIpAddr(request));
     ...
     * @return
     */
    public Object wxToPay(Integer userId,
                        BigDecimal payMoney, HttpServletRequest request,HttpServletResponse response) {
        System.out.println("充值卡支付");
        ...
        try {
            ...
            String openid = user.getWeixinOpenid();
            if (openid == null) {
                return ResponseUtil.fail(AUTH_OPENID_UNACCESS, "获取不到微信OpenId,订单不能支付");
            }
...
//        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
    ...
    private Object operatOrder(String orderSn,String payId,String totalFee,String openid){
         ...
         //判断订单是否关联拍卖出价记录，如果关联，释放该订单的保证金

        if(!StringUtils.isEmpty(order.getAuctionType())&&(order.getAuctionType().equals("大家拍")||order.getAuctionType().equals("专场拍"))) {
            userChargeMoneyLockService.handleUnLockOrder(order);
        }
...
//       notifyService.notifySmsTemplate(order.getMobile(), NotifyType.PAY_SUCCEED, new String[]{orderSn.substring(8, 14)});
    ...
    private Object operatCharge(String orderSn,String payId,String totalFee,String openid){
        ...
        if(user!=null){
            user.setChargeRemainMoney(user.getChargeRemainMoney().add(userChargeMoney.getChargeMoney()));
            user.setChargeSumMoney(user.getChargeSumMoney().add(userChargeMoney.getChargeMoney()));
...
//        opadminInfoService.addLitemallOpadminInfo(opadminInfo,parameters);
    ...
    /**
     * 判断用户是否已经锁定了拍卖保证金，如果已经锁定，则可以继续出价，如果没有锁定，需要交保证金
     */
...
//        String appsecret = wxMaConfig.getSecret();
	...
	public void alipaySuccess(String ordersn, String payId) {
	    if(ordersn.startsWith("C")){
            alipaySuccessChargeMoney(ordersn,payId);
            return;
        }
		LitemallOrder order = orderService.findBySn(ordersn);
		if (order == null) {
...
//    	        notifyService.notifyWxTemplate(.getOpenid(), NotifyType.PAY_SUCCEED, parms, "pages/index/index?orderId=" + order.getId());
     ...
     * @param payId
     */
    public void alipaySuccessChargeMoney(String ordersn, String payId) {
        LitemallUserChargeMoney userChargeMoney = userChargeMoneyService.queryByChargeSheetNo(ordersn);
        if (userChargeMoney == null) {
            return ;
        }
...
//        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
    ...
    public Object submitPrivateMakeOrder(Integer userId,
        ...
        //向订单明细中表增加私人定制的品项信息
        LitemallPrivateMakeRule privateMakeRule= privateMakeRuleService.findById(makeOrder.getRuleMxId());
        LitemallOrderGoods orderGoods=new LitemallOrderGoods();
        orderGoods.setOrderId(order.getId());
        orderGoods.setGoodsId(privateMakeRule.getGoodsId());
        orderGoods.setProductId(privateMakeRule.getGoodsProductId());
        orderGoods.setGoodsName(privateMakeRule.getGoodsName());
        orderGoods.setGoodsSn(privateMakeRule.getGoodsSn());
        orderGoods.setNumber(Short.parseShort("1"));
        orderGoods.setPrice(privateMakeRule.getDingjinPrice());
        String[] strSpe=(String[]) JSON.parseArray(privateMakeRule.getGoodsProductSpecifications().replaceAll("\\\\","")).toArray(new String[0]);
        orderGoods.setSpecifications(strSpe);
        orderGoods.setPicUrl(privateMakeRule.getPrivatePicHead());
        orderGoodsService.add(orderGoods);
