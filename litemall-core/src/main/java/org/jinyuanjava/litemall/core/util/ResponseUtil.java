...
public class ResponseUtil {
    ...
    public static Object okList(List list, Integer total,Integer pageNumber,Integer pageSize) {
        Map<String, Object> data = new HashMap<String, Object>();
        data.put("list", list);
        data.put("total", total);
        data.put("page", pageNumber);
        data.put("limit",pageSize);
        if (total == -1L||pageSize==0) {
            data.put("pages", 0);
        } else {
            if (pageSize > 0) {
                int pages = (int)(total / (long)pageSize + (long)(total % (long)pageSize == 0L ? 0 : 1));
    ...
    //已经微信登陆，但还没有注册手机号
    public static Object loginButNoMobile() {
        return fail(999, "请用手机号注册");
    }
